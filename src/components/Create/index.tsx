...
import { ROOT, TEST_CATEGORIES, TEST_ELEMENTS, TEST_VARIATIONS } from "../../constants";
import { ABTest, getTests, createTest, deleteTest } from "../../client/abtest";
...
const Create = () => {
  const { register, handleSubmit, watch, formState: { errors }, getValues, setValue } = useForm<IExperimentParameters>({
    defaultValues: {
      id: `ex${getUnixTime()}`,
      name: "",
  ...
  const constructTest = ({ id, name, trigger, sampleRate, Original, Variant }: IExperimentParameters) => ( 
`Mojito.addTest({
  id: "${id}",
  name: "${name}",
  ...
  return (
    <div className="p-5 bg-main min-w-screen min-h-screen">
      <form className="container mx-auto flex flex-col items-center" onSubmit={handleSubmit(generateTest)}>
          ...
          <section className={`mb-2 grid grid-cols-3 gap-2`}>
            ...
            <section className={`flex flex-col`}>
              ...
              <input
                ...
                min={0.0}
              />
              {errors.sampleRate && <p className="text-red-500 text-xs">{errors.sampleRate.message}</p>}
            </section>
          </section>
        </section>
              ...
              <ul className="flex flex-wrap">
                  <li className="mr-2">
                      <button type={"button"} 
                        ...
                        className={`inline-block p-3 ${tab === "Original" ? "active dark:text-blue-500 dark:border-blue-500" : "hover:text-gray-500 hover:border-gray-300 dark:hover:text-gray-300"}`}
                      >
                        Original (A)
                      </button>
                  </li>
                  <li className="mr-2">
                      <button type={"button"}
                        ...
                        className={`inline-block p-3 ${tab === "Variant" ? "active dark:text-blue-500 dark:border-blue-500 hover" : "hover:text-gray-500 hover:border-gray-300 dark:hover:text-gray-300"}`}
                      >
                        Variant (B)
                      </button>
                  </li>
          ...
          <div className="text-sm my-2">
            {
                tab === "Original" && (
                  <section className={`mb-2 grid grid-cols-2 gap-2`}>
                    <section className={`flex flex-col`}>
                      <label className="text-label text-sm mb-3">Original JavaScript</label>
                      <textarea
                        rows={4}
                        className="border border-text-input rounded py-3 px-4 text-sm text-black"
                    ...
                    <section className={`flex flex-col`}>
                      <label className="text-label text-sm mb-3">Original CSS</label>
                      <textarea
                        rows={4}
                        className="border border-text-input rounded py-3 px-4 text-sm text-black"
              ...
              }
              {
                tab === "Variant" && (
                  <section className={`mb-2 grid grid-cols-2 gap-2`}>
                    <section className={`flex flex-col`}>
                      <label className="text-label text-sm mb-3">Variant JavaScript</label>
                      <textarea
                        rows={4}
                        className="border border-text-input rounded py-3 px-4 text-sm text-black"
          ...
          >
            {
              ...
              <section className={`mb-2 flex flex-col`}>
                  ...
                  />
                  {errors.trigger && <p className="text-red-500 text-xs">{errors.trigger.message}</p>}
              </section>
            }
            {
