...
const History = () => {
    ...
    const constructScripts = (sptr: string, mojito: string, test: string, name: string) => (
`<!-- ${name} -->
<!-- Snowplow -->
...
<!-- End of ${name} -->`
    ...
    const handleConstructTags = (test: ABTest) => {
      const s = constructScripts(`${ROOT}/${snowplowDefault}`, `${ROOT}/${snowplowMojito}`, `${ROOT}/${test.s3_key}`, test.title);
      setScripts(s);
    }
    useEffect(() => {
        ...
        if (tokens) {
          setUserData(tokens);
          getTests(tokens.id_token).then((tests) => {
  ...
  return (
    <div className="p-5 bg-main min-w-screen min-h-screen">
      <div className="container mx-auto flex flex-col items-center mt-5">
          <div className="my-4">
            <h2 className="text-2xl font-bold dark:text-dark">User Test History</h2>
          </div>
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  {/* <th scope="col" className="px-4 py-3">Get</th> */}
              ...
              <tbody>
                  {
                    testHistory && testHistory.map((test, index) => (
                      ...
                      >
                        <td className="px-4 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{test.title}</td>
                        <td className="px-4 py-4">{new Date(test.iso_created_at).toISOString().substring(0, 16).replace("T", " ")}</td>
                        ...
                        {/* <td className="px-4 py-4">
                          <button
                            ...
                            onClick={() => handleCopyScript(test.s3_key)}
                            style={{cursor: "pointer"}}
                            >get</button>
                        </td> */}
                        <td className="px-4 py-4">
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (ok) {
                                userData && deleteTest(userData.id_token, test.record_id)
                                .then((res) => {
          ...
          <div className="inline-block text-center">
            <p className="text-base text-black" style={{padding: "12px"}}><b>User:</b> {userData && userData.email}</p>
          </div>
          <Button
              text="Logout"
              onClick={() => {
